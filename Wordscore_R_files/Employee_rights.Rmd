...
```{r}
comments <- read.xlsx("comments_data.xlsx", sheet = "Employee_rights")
```
...
```{r}
# Run Wordfish model
wf_quanteda <- textmodel_wordfish(dfm_q)
```
...
ggplot(plot_data, aes(x = theta, y = Organisation, color = Group)) +
  ...
  labs(
    title = "Notification of Employee Rights under the NLRA",
...
```{r}
#textplot_scale1d(wf_quanteda, groups = docvars(dfm_0q, "Labor"))
```
