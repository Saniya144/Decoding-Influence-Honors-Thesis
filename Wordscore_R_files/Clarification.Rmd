...
```{r}
comments <- read.xlsx("comments_data.xlsx", sheet = "Clarification")
```
...
```{r}
# Run Wordfish model
wf_quanteda <- textmodel_wordfish(dfm_q, dir = c(9, 1))
```
...
ggplot(plot_data, aes(x = theta, y = Organisation, color = Group)) +
  ...
  labs(
    title = "Employer's Continuing Obligation to Maintain Record Injury",
